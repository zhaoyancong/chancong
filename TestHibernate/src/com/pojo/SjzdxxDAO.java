...
public class SjzdxxDAO extends BaseHibernateDAO
{
	private static final Logger log = LoggerFactory.getLogger(SjzdxxDAO.class);
	// property constants
	...
	public Sjzdxx findById(java.lang.Integer id)
	{
		...
		try
		{
			Sjzdxx instance = (Sjzdxx) getSession().get("com.pojo.Sjzdxx", id);
			return instance;
		}
		catch (RuntimeException re)
		{
	...
	public List findByExample(Sjzdxx instance)
	{
		...
		try
		{
			List results = getSession().createCriteria("com.pojo.Sjzdxx").add(Example.create(instance)).list();
	...
	{
		log.debug("finding Sjzdxx instance with property: " + propertyName + ", value: " + value);
		try
		{
			String queryString = "from Sjzdxx as model where model." + propertyName + "= ?";
