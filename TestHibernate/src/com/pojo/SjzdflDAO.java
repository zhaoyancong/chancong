...
{
	private static final Logger log = LoggerFactory.getLogger(SjzdflDAO.class);
	// property constants
	...
	public Sjzdfl findById(java.lang.Integer id)
	{
		...
		try
		{
			Sjzdfl instance = (Sjzdfl) getSession().get("com.pojo.Sjzdfl", id);
			return instance;
		}
		catch (RuntimeException re)
		{
	...
	public List findByExample(Sjzdfl instance)
	{
		...
		try
		{
			List results = getSession().createCriteria("com.pojo.Sjzdfl").add(Example.create(instance)).list();
	...
	{
		log.debug("finding Sjzdfl instance with property: " + propertyName + ", value: " + value);
		try
		{
			String queryString = "from Sjzdfl as model where model." + propertyName + "= ?";
